...
MONITOR_SCRIPT = HERE.parent / "nsx-monitor" / "nsx-monitor.py"
REPORTS_DIR = DATA_DIR / "reports"

# Fallback: if not found at expected path, try /root/nsx-monitor/
if not MONITOR_SCRIPT.exists():
    ALT = Path("/root/nsx-monitor/nsx-monitor.py")
    if ALT.exists():
        MONITOR_SCRIPT = ALT

# Also check just "nsx-monitor.py" in the current directory (dev mode)
if not MONITOR_SCRIPT.exists():
    LOCAL = HERE / "nsx-monitor.py"
    if LOCAL.exists():
        MONITOR_SCRIPT = LOCAL
